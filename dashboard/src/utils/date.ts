export function getDiffTimeBetweenCurrentDate (dateString: any, now = new Date()) {
  ...
  const buildMessage = (label: any, value: any) => {
    if (value !== 1) {
  ...
  const notZero = (value: any) => value !== 0

  if (month !== now.getMonth()) {
    const diff = Math.abs((now as any) - (date as any))
